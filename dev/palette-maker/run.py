...
def show_color(r, g, b):
    for i in range(18):  
        n[i] = (min(round(r * brightness / 255.0), 255),
                min(round(g * brightness / 255.0), 255),
                min(round(b * brightness / 255.0), 255))
    print("brightness=",round(brightness),"rgb=",n[0])
    n.write()
...
while True:
    ...
    led.toggle()
    brightness = float(adc_values[KNOB_BREAK_ATTEN]) / 1023.0 * 255.0
    show_color(round(adc_values[KNOB_BREAK]/4.0),round(adc_values[KNOB_AMEN]/4.0),round(adc_values[KNOB_SAMPLE]/4.0))
    time.sleep(0.1)
