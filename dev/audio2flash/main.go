...
`)
	...
	sb.WriteString(fmt.Sprintf("bool %s_played = false;\n", codeNameLower))
	sb.WriteString(fmt.Sprintf("bool %s_audio_update(int32_t* samples, uint16_t num_samples, uint vol_main) {\n", codeNameLower))
	sb.WriteString(fmt.Sprintf("  if (!%s_is_playing && %s_do_play<0) {\n", codeNameLower, codeNameLower))
	sb.WriteString(fmt.Sprintf("return %s_played;\n", codeNameLower))
	...
	sb.WriteString(fmt.Sprintf("  int32_t audio = q16_16_multiply_cue((cuedsounds_audio[cuedsounds_index]) << 16, vol_main);\n"))
	...
	sb.WriteString(fmt.Sprintf(" %s_played = true;\n", codeNameLower))
