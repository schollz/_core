...
static void in_spi_irq_handler(const uint DMA_IRQ_num,
...
bool spi_transfer(spi_t *spi_p, const uint8_t *tx, uint8_t *rx, size_t length) {
  ...
  bool rc = sem_acquire_timeout_ms(&spi_p->sem,
...
bool my_spi_init(spi_t *spi_p) {
  ...
  if (!spi_p->initialized) {
    ...
    // Defaults:
    if (!spi_p->baud_rate) spi_p->baud_rate = 10 * 1000 * 1000;
    ...
    if (spi_p->set_drive_strength) {
      gpio_set_drive_strength(spi_p->mosi_gpio,
                              spi_p->mosi_gpio_drive_strength);
    ...
    // asserted:
    if (!spi_p->DMA_IRQ_num) spi_p->DMA_IRQ_num = DMA_IRQ_0;
    irq_add_shared_handler(spi_p->DMA_IRQ_num, spi_irq_handler,
