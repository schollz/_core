...
static bool logSDError(int line) {
  ...
  azlog("SDIO SD card error on line ", line, ", error code ",
        (int)g_sdio_error);
  return false;
}
...
static sd_callback_t get_stream_callback(const uint8_t *buf, uint32_t count,
  ...
  if (m_stream_callback) {
    if (buf == m_stream_buffer + m_stream_count) {
      ...
      azdbg("SD card ", accesstype, "(", (int)sector, ") slow transfer, buffer",
            (uint32_t)buf, " vs. ",
...
bool sd_sdio_begin(sd_card_t *sd_card_p) {
  ...
  for (int retries = 0; retries < 5; retries++) {
    ...
    status =
        rp2040_sdio_command_R1(sd_card_p, CMD8, 0x1AA, &reply);  // SEND_IF_COND
  ...
  do {
    if (!checkReturnOk(
            rp2040_sdio_command_R1(sd_card_p, CMD55, 0, &reply)) ||  // APP_CMD
        !checkReturnOk(rp2040_sdio_command_R3(sd_card_p, ACMD41, 0xD0040000,
  ...
  // Get CID
  if (!checkReturnOk(
          rp2040_sdio_command_R2(sd_card_p, CMD2, 0, (uint8_t *)&g_sdio_cid))) {
  ...
  // Get CSD
  if (!checkReturnOk(rp2040_sdio_command_R2(sd_card_p, CMD9, g_sdio_rca,
                                            (uint8_t *)&g_sdio_csd))) {
  ...
  // Select card
  if (!checkReturnOk(
          rp2040_sdio_command_R1(sd_card_p, CMD7, g_sdio_rca, &reply))) {
  ...
  // Set 4-bit bus mode
  if (!checkReturnOk(
          rp2040_sdio_command_R1(sd_card_p, CMD55, g_sdio_rca, &reply)) ||
...
uint32_t sd_sdio_kHzSdClk(sd_card_t *sd_card_p) { return 0; }
...
bool sd_sdio_readOCR(sd_card_t *sd_card_p, uint32_t *ocr) {
  ...
  return checkReturnOk(
      rp2040_sdio_command_R1(sd_card_p, CMD13, g_sdio_rca, ocr));
}
...
uint32_t sd_sdio_status(sd_card_t *sd_card_p) {
  uint32_t reply;
  if (checkReturnOk(
          rp2040_sdio_command_R1(sd_card_p, CMD13, g_sdio_rca, &reply)))
    return reply;
  else
    return 0;
}
...
bool sd_sdio_syncDevice(sd_card_t *sd_card_p) { return true; }
...
bool sd_sdio_erase(sd_card_t *sd_card_p, uint32_t firstSector,
                   uint32_t lastSector) {
...
bool sd_sdio_writeSector(sd_card_t *sd_card_p, uint32_t sector,
  ...
  if (/* !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, 16, 512, &reply)) ||
         // SET_BLOCKLEN */
      !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD24, sector,
                                            &reply)) ||  // WRITE_BLOCK
      !checkReturnOk(
...
bool sd_sdio_writeSectors(sd_card_t *sd_card_p, uint32_t sector,
  ...
  if (/* !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, 16, 512, &reply)) ||
         // SET_BLOCKLEN */
      !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD55, g_sdio_rca,
                                            &reply)) ||  // APP_CMD
      !checkReturnOk(rp2040_sdio_command_R1(
          sd_card_p, ACMD23, n, &reply)) ||  // SET_WR_CLK_ERASE_COUNT
      !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD25, sector,
  ...
  if (g_sdio_error != SDIO_OK) {
    azlog("sd_sdio_writeSectors(", sector, ",...,", (int)n,
...
static int sd_sdio_write_blocks(sd_card_t *sd_card_p, const uint8_t *buffer,
  ...
  // bool sd_sdio_writeSectors(sd_card_t *sd_card_p, uint32_t sector, const
  // uint8_t* src, size_t ns);
  bool rc;
  if (1 == blockCnt)
...
static int sd_sdio_read_blocks(sd_card_t *sd_card_p, uint8_t *buffer,
                               uint64_t ulSectorNumber,
                               uint32_t ulSectorCount) {
  // bool sd_sdio_readSectors(sd_card_t *sd_card_p, uint32_t sector, uint8_t*
  // dst, size_t n)
  bool rc;
  if (1 == ulSectorCount)
...
void sd_sdio_ctor(sd_card_t *sd_card_p) {
  ...
  sd_card_p->sdio_if.CLK_gpio =
      (sd_card_p->sdio_if.D0_gpio + SDIO_CLK_PIN_D0_OFFSET) % 32;
