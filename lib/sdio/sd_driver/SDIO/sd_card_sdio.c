...
bool sd_sdio_begin(sd_card_t *sd_card_p) {
  ...
  if (!rp2040_sdio_init(sd_card_p, calculate_clk_div(400 * 1000))) return false;

  // Establish initial connection with the card
  for (int retries = 0; retries < 5; retries++) {
    delay_ms(1);
    reply = 0;
    rp2040_sdio_command_R1(sd_card_p, CMD0_GO_IDLE_STATE, 0,
                           NULL);  // GO_IDLE_STATE
    status = rp2040_sdio_command_R1(sd_card_p, CMD8_SEND_IF_COND, 0x1AA,
                                    &reply);  // SEND_IF_COND
  ...
  if (reply != 0x1AA || status != SDIO_OK) {
    // azdbg("SDIO not responding to CMD8 SEND_IF_COND, status ", (int)status, "
    // reply ", reply);
    EMSG_PRINTF(
        "%s,%d SDIO not responding to CMD8 SEND_IF_COND, status 0x%x reply "
        "0x%lx\n",
  ...
  do {
    if (!checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD55_APP_CMD, 0,
                                              &reply)) ||  // APP_CMD
        !checkReturnOk(rp2040_sdio_command_R3(sd_card_p, ACMD41_SD_SEND_OP_COND,
                                              0xD0040000,
                                              &STATE.ocr)))  // 3.0V voltage
    // !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, ACMD41, 0xC0100000,
    // &STATE.ocr)))
    {
      return false;
    }
  ...
  if (!checkReturnOk(rp2040_sdio_command_R2(
          sd_card_p, CMD2_ALL_SEND_CID, 0, (uint8_t *)&sd_card_p->state.CID))) {
  ...
  if (!checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD3_SEND_RELATIVE_ADDR,
                                            0, &STATE.rca))) {
  ...
  if (!checkReturnOk(rp2040_sdio_command_R2(sd_card_p, CMD9_SEND_CSD, STATE.rca,
                                            sd_card_p->state.CSD))) {
  ...
  if (!checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD7_SELECT_CARD,
                                            STATE.rca, &reply))) {
  ...
  For Mode Selection, the host can drive the line high or let it be pulled high
  to select SD mode. If the host wants to select SPI mode it should drive the
  line low. For Card detection, the host detects that the line is pulled high.
  ...
  if (!checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD55_APP_CMD, STATE.rca,
                                            &reply)) ||
      !checkReturnOk(rp2040_sdio_command_R1(
          sd_card_p, ACMD42_SET_CLR_CARD_DETECT, 0, &reply))) {
  ...
  if (!checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD55_APP_CMD, STATE.rca,
                                            &reply)) ||
      !checkReturnOk(
          rp2040_sdio_command_R1(sd_card_p, ACMD6_SET_BUS_WIDTH, 2, &reply))) {
    azdbg("SDIO failed to set bus width");
    return false;
  }
  if (!checkReturnOk(
          rp2040_sdio_command_R1(sd_card_p, 16, 512, &reply)))  // SET_BLOCKLEN
  {
  ...
  if (!rp2040_sdio_init(sd_card_p,
                        calculate_clk_div(sd_card_p->sdio_if_p->baud_rate)))
    return false;
...
bool sd_sdio_readOCR(sd_card_t *sd_card_p, uint32_t *ocr) {
  ...
  return checkReturnOk(
      rp2040_sdio_command_R1(sd_card_p, CMD13_SEND_STATUS, STATE.rca, ocr));
}

uint32_t sd_sdio_status(sd_card_t *sd_card_p) {
  uint32_t reply;
  if (checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD13_SEND_STATUS,
                                           STATE.rca, &reply)))
    return reply;
  else
    return 0;
...
bool sd_sdio_stopTransmission(sd_card_t *sd_card_p, bool blocking) {
  ...
  uint32_t reply;
  if (!checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD12_STOP_TRANSMISSION,
                                            0, &reply))) {
    return false;
  }
...
bool sd_sdio_writeSector(sd_card_t *sd_card_p, uint32_t sector,
                         const uint8_t *src) {
  ...
  if (/* !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, 16, 512, &reply)) ||
         // SET_BLOCKLEN */
      !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD24_WRITE_BLOCK,
                                            sector, &reply)) ||  // WRITE_BLOCK
      !checkReturnOk(
          rp2040_sdio_tx_start(sd_card_p, src, 1)))  // Start transmission
  {
    return false;
  }
...
bool sd_sdio_writeSectors(sd_card_t *sd_card_p, uint32_t sector,
  ...
  if (STATE.ongoing_wr_mlt_blk && sector == STATE.wr_mlt_blk_cnt_sector) {
    ...
    if (!checkReturnOk(
            rp2040_sdio_tx_start(sd_card_p, src, n)))  // Start transmission
      return false;
  } else {
    ...
    uint32_t reply;
    if (!checkReturnOk(rp2040_sdio_command_R1(
            sd_card_p, CMD25_WRITE_MULTIPLE_BLOCK, sector, &reply)) ||
        !checkReturnOk(
            rp2040_sdio_tx_start(sd_card_p, src, n)))  // Start transmission
    {
      return false;
...
bool sd_sdio_readSector(sd_card_t *sd_card_p, uint32_t sector, uint8_t *dst) {
  ...
  if (/* !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, 16, 512, &reply)) ||
         // SET_BLOCKLEN */
      !checkReturnOk(rp2040_sdio_rx_start(
          sd_card_p, dst, 1, SDIO_BLOCK_SIZE)) ||  // Prepare for reception
      !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD17_READ_SINGLE_BLOCK,
                                            sector,
...
bool sd_sdio_readSectors(sd_card_t *sd_card_p, uint32_t sector, uint8_t *dst,
  ...
  if (sector + n > sd_card_p->state.sectors) {
    EMSG_PRINTF("sd_sdio_readSectors: sector range exceeds drive capacity\n");
    return false;
  }

  // Handle unaligned reads with optimized single-sector path
  if (((uint32_t)dst & 3) != 0) {
    // Use optimized single sector reads for small unaligned transfers
    if (n <= 4) {
      for (size_t i = 0; i < n; i++) {
    ...
    // For larger unaligned reads, use aligned temporary buffer strategy
    uint8_t *aligned_buf = (uint8_t *)STATE.dma_buf;
    size_t sectors_per_batch = sizeof(STATE.dma_buf) / 512;

    for (size_t i = 0; i < n; i += sectors_per_batch) {
      size_t batch_size =
          (i + sectors_per_batch <= n) ? sectors_per_batch : (n - i);

      if (!sd_sdio_readSectors(sd_card_p, sector + i, aligned_buf,
                               batch_size)) {
        return false;
      }

      memcpy(dst + i * 512, aligned_buf, batch_size * 512);
    }
    return true;
  }
  ...
  uint32_t reply;
  if (!checkReturnOk(
          rp2040_sdio_rx_start(sd_card_p, dst, n, SDIO_BLOCK_SIZE)) ||
      !checkReturnOk(rp2040_sdio_command_R1(
          sd_card_p, CMD18_READ_MULTIPLE_BLOCK, sector, &reply))) {
    return false;
  }

  // Optimized polling with adaptive delay
  uint32_t poll_count = 0;
  do {
    STATE.error = rp2040_sdio_rx_poll(sd_card_p, SDIO_WORDS_PER_BLOCK);
    if (STATE.error == SDIO_BUSY && (++poll_count & 0x3F) == 0) {
      // Every 64 polls, yield briefly to prevent CPU starvation
      sleep_us(1);
    }
  } while (STATE.error == SDIO_BUSY);

  if (STATE.error != SDIO_OK) {
    EMSG_PRINTF("sd_sdio_readSectors(%ld,...,%zu) failed: %s (%d)\n", sector, n,
                errstr(STATE.error), STATE.error);
    sd_sdio_stopTransmission(sd_card_p, true);
    return false;
  }
...
bool rp2040_sdio_get_sd_status(sd_card_t *sd_card_p, uint8_t response[64]) {
  uint32_t reply;
  if (!checkReturnOk(rp2040_sdio_rx_start(sd_card_p, response, 1,
                                          64)) ||  // Prepare for reception
      !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, CMD55_APP_CMD, STATE.rca,
                                            &reply)) ||  // APP_CMD
      !checkReturnOk(rp2040_sdio_command_R1(sd_card_p, ACMD13_SD_STATUS, 0,
                                            &reply)))  // SD Status
  {
...
static bool sd_sdio_test_com(sd_card_t *sd_card_p) {
  ...
  if (!(sd_card_p->state.m_Status & STA_NOINIT)) {
    ...
    uint32_t reply = 0;
    sdio_status_t status =
        rp2040_sdio_command_R1(sd_card_p, CMD13_SEND_STATUS, STATE.rca, &reply);
    ...
    sdio_status_t status = rp2040_sdio_command_R1(
        sd_card_p, CMD8_SEND_IF_COND, 0x1AA, &reply);  // SEND_IF_COND
...
static void gpio_conf(uint gpio, gpio_function_t fn, bool pullup, bool pulldown,
                      bool output, bool initial_state) {
...
static DSTATUS sd_sdio_init(sd_card_t *sd_card_p) {
  ...
  //        pin                             function        pup   pdown  out
  //        state
  gpio_conf(sd_card_p->sdio_if_p->CLK_gpio, GPIO_FUNC_PIO1, true, false, true,
            true);
  gpio_conf(sd_card_p->sdio_if_p->CMD_gpio, GPIO_FUNC_PIO1, true, false, true,
            true);
  gpio_conf(sd_card_p->sdio_if_p->D0_gpio, GPIO_FUNC_PIO1, true, false, false,
            true);
  gpio_conf(sd_card_p->sdio_if_p->D1_gpio, GPIO_FUNC_PIO1, true, false, false,
            true);
  gpio_conf(sd_card_p->sdio_if_p->D2_gpio, GPIO_FUNC_PIO1, true, false, false,
            true);
  gpio_conf(sd_card_p->sdio_if_p->D3_gpio, GPIO_FUNC_PIO1, true, false, false,
            true);
...
static void sd_sdio_deinit(sd_card_t *sd_card_p) {
  ...
  //        pin                             function        pup   pdown   out
  //        state
  gpio_conf(sd_card_p->sdio_if_p->CLK_gpio, GPIO_FUNC_NULL, false, false, false,
            false);
  gpio_conf(sd_card_p->sdio_if_p->CMD_gpio, GPIO_FUNC_NULL, false, false, false,
            false);
  gpio_conf(sd_card_p->sdio_if_p->D0_gpio, GPIO_FUNC_NULL, false, false, false,
            false);
  gpio_conf(sd_card_p->sdio_if_p->D1_gpio, GPIO_FUNC_NULL, false, false, false,
            false);
  gpio_conf(sd_card_p->sdio_if_p->D2_gpio, GPIO_FUNC_NULL, false, false, false,
            false);
  gpio_conf(sd_card_p->sdio_if_p->D3_gpio, GPIO_FUNC_NULL, false, false, false,
            false);
...
static block_dev_err_t sd_sdio_write_blocks(sd_card_t *sd_card_p,
...
static block_dev_err_t sd_sdio_read_blocks(sd_card_t *sd_card_p,
                                           uint8_t *buffer,
                                           uint32_t ulSectorNumber,
