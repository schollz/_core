...
inline static audio_buffer_t *list_remove_head_with_tail(
    audio_buffer_t **phead, audio_buffer_t **ptail) {
  audio_buffer_t *ab = *phead;
...
inline static void list_append_with_tail(audio_buffer_t **phead,
...
audio_buffer_t *get_full_audio_buffer(audio_buffer_pool_t *context,
  ...
  do {
    ...
    ab = list_remove_head_with_tail(&context->prepared_list,
                                    &context->prepared_list_tail);
...
void queue_full_audio_buffer(audio_buffer_pool_t *context, audio_buffer_t *ab) {
  assert(!ab->next);
  uint32_t save = spin_lock_blocking(context->prepared_list_spin_lock);
  list_append_with_tail(&context->prepared_list, &context->prepared_list_tail,
                        ab);
  spin_unlock(context->prepared_list_spin_lock, save);
  __sev();
}
...
audio_buffer_t *producer_pool_take_buffer_default(
    audio_connection_t *connection, bool block) {
...
audio_buffer_t *consumer_pool_take_buffer_default(
    audio_connection_t *connection, bool block) {
...
void audio_init_buffer(audio_buffer_t *audio_buffer,
                       audio_buffer_format_t *format, int buffer_sample_count) {
  audio_buffer->format = format;
  audio_buffer->buffer =
      pico_buffer_alloc(buffer_sample_count * format->sample_stride);
  audio_buffer->max_sample_count = buffer_sample_count;
  audio_buffer->sample_count = 0;
}

audio_buffer_pool_t *audio_new_buffer_pool(audio_buffer_format_t *format,
                                           int buffer_count,
                                           int buffer_sample_count) {
  audio_buffer_pool_t *ac =
      (audio_buffer_pool_t *)calloc(1, sizeof(audio_buffer_pool_t));
  audio_buffer_t *audio_buffers =
      buffer_count
          ? (audio_buffer_t *)calloc(buffer_count, sizeof(audio_buffer_t))
          : 0;
  ac->format = format->format;
  for (int i = 0; i < buffer_count; i++) {
    audio_init_buffer(audio_buffers + i, format, buffer_sample_count);
    audio_buffers[i].next =
        i != buffer_count - 1 ? &audio_buffers[i + 1] : NULL;
  }
  ac->free_list_spin_lock = spin_lock_init(SPINLOCK_ID_AUDIO_FREE_LIST_LOCK);
  ac->free_list = audio_buffers;
  ac->prepared_list_spin_lock =
      spin_lock_init(SPINLOCK_ID_AUDIO_PREPARED_LISTS_LOCK);
...
audio_buffer_t *audio_new_wrapping_buffer(audio_buffer_format_t *format,
  ...
  audio_buffer_t *audio_buffer =
      (audio_buffer_t *)calloc(1, sizeof(audio_buffer_t));
  if (audio_buffer) {
    audio_buffer->format = format;
...
audio_buffer_pool_t *audio_new_producer_pool(audio_buffer_format_t *format,
                                             ...
                                             int buffer_sample_count) {
  audio_buffer_pool_t *ac =
      audio_new_buffer_pool(format, buffer_count, buffer_sample_count);
...
audio_buffer_t *stereo_s16_to_stereo_s16_consumer_take(
    audio_connection_t *connection, bool block) {
  return consumer_pool_take<Stereo<FmtS16>, Stereo<FmtS16>>(connection, block);
}

audio_buffer_t *stereo_s32_to_stereo_s32_consumer_take(
    audio_connection_t *connection, bool block) {
...
void stereo_s16_to_stereo_s16_producer_give(audio_connection_t *connection,
                                            audio_buffer_t *buffer) {
  return producer_pool_blocking_give<Stereo<FmtS16>, Stereo<FmtS16>>(connection,
                                                                     buffer);
}

void stereo_s32_to_stereo_s32_producer_give(audio_connection_t *connection,
                                            audio_buffer_t *buffer) {
  return producer_pool_blocking_give<Stereo<FmtS32>, Stereo<FmtS32>>(connection,
                                                                     buffer);
}