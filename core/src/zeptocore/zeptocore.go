...
func (f File) Regenerate() {
	fu := func() {
		// Lock this specific file's regeneration
		f.regenMutex.Lock()
		defer f.regenMutex.Unlock()
...
func processSound(fnameIn string, fnameOut string, channels int, oversampling int) (err error) {
	...
	_, _, err = utils.Run(sox.GetBinary(), "-t", "raw", "-c", fmt.Sprint(channels), "-r", fmt.Sprint(44100*oversampling), "--bits", "16", "--encoding", "signed-integer", "--endian", "little", tempRaw, fnameOut)
	if err != nil {
		log.Error(err)
		return
	}
